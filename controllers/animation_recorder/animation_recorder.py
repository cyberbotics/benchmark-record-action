...
def main():
    ...
    parser.add_argument('--metric', default='percent', help='List of controllers to run')
    ...
    for controller in controllers:
        ...
        while not done and robot.step(timestep) != -1 and step_i < n_steps:
            ...
            if receiver.getQueueLength() > 0:
                print(receiver.getData().decode('utf-8'))
        ...
        elif args.metric != 'time-duration':
            performance_line = controller.split('_')[1] + ':0:failed:' + datetime.today().strftime('%Y-%m-%d')
            message = "Benchmark failed: time limit reached."
        else:
            message = "Benchmark completed with time limit."
            competitor_id = controller.split('_')[1]
            performance = str(args.duration)
            performance_string = time_convert(args.duration)
            performance_line = competitor_id + ':' + performance + ':' + performance_string + ':' + datetime.today().strftime('%Y-%m-%d')
